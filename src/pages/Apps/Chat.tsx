...
const Chat = () => {
    ...
    useEffect(() => {
        ...
        updateMessagesList();
        const interval = setInterval(updateMessagesList, 5000);
        setBackgroundPolling(interval);
    ...
    useEffect(() => {
        if (location.state?.phoneNumber) {
            let user = contactList.find((u: any) => u.userId === location.state.phoneNumber);
            if (!user) {
                user = {
                    userId: location.state.phoneNumber,
                    name: location.state.phoneNumber,
    ...
    const updateRequestStatus = async (userId: string, status: number) => {
        try {
            await postRequest("/v1/whatsapp/updateRequestStatus", { userId, request_status: status }, {}, headers);
        } catch (e) {}
    };
    ...
    const selectUser = async (user: any) => {
        if (eventSource) eventSource.close();
        if (pollingInterval) clearInterval(pollingInterval);
        await updateRequestStatus(user.userId, 1);
